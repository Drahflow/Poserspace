...
#define MAXLINES ((HEIGHT - LINEHEIGHT) / LINEHEIGHT)
...
class GeoInterpreter: public DataInterpreter {
  public:
    virtual void handleData(const std::vector<std::string> &values) {
      std::cerr << "Geo: " << values[0] << "," << values[1] << std::endl;
...
void renderFrame(SDL_Surface *screen, SDL_Renderer *renderer) {
  ...
  {
    int y = LINEHEIGHT / 2;
    for(auto l: geo.lines) {
      SDL_Surface *text = TTF_RenderUTF8_Blended(font, l.c_str(), SDL_Color{0, 255, 0, 128});
      if(!text) throw "TTF_RenderUTF8_Solid failed";

      SDL_Rect r{100, y, text->w, text->h};
      SDL_BlitSurface(text, nullptr, screen, &r);
...
int main(int, char **) {
  try {
    ...
    font = TTF_OpenFont("hackers.ttf", FONTSIZE);
    if(!font) throw "TTF_OpenFont failed";
